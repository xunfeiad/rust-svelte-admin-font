<script lang="ts">
	...
	const handleRemember = ()=>{
	   const username = localStorage.set("username", );
			...
			<form class="mt-8 space-y-6" on:submit|preventDefault>
				...
				{#if rememberMe || lostPassword}
					<div class="flex items-start">
						{#if rememberMe}
							<Checkbox class="accent-primary-600" name="remember" onclick="{handleRemember}">Remember me</Checkbox>
						{/if}
						{#if lostPassword}
